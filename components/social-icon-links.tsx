...
function SocialGlyph({ id }: { id: PublicSocialLink["id"] }) {
  const svgClass = "h-3.5 w-3.5 shrink-0";
  if (id === "instagram") {
    return (
      <svg viewBox="0 0 24 24" className={svgClass} fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden>
...
export function SocialIconLinks({
  ...
  return (
    <div className={`flex flex-nowrap items-center ${className ?? ""}`}>
      {links.map((link) => {
        if (link.placeholder) {
          return (
            <span
              ...
              className={`${iconClassName} shrink-0 cursor-default opacity-90`}
            >
