-- Configurable standard delivery + optional free delivery when cart subtotal (paisa) meets threshold.
-- Values edited in admin Settings; enforced in public.place_order.

alter table public.store_settings
  add column if not exists standard_delivery_paisa integer not null default 50000
    constraint store_settings_standard_delivery_paisa_range check (
      standard_delivery_paisa >= 0 and standard_delivery_paisa <= 2000000000
    );

alter table public.store_settings
  add column if not exists free_delivery_min_subtotal_paisa integer
    constraint store_settings_free_delivery_min_range check (
      free_delivery_min_subtotal_paisa is null
      or (
        free_delivery_min_subtotal_paisa >= 0
        and free_delivery_min_subtotal_paisa <= 2000000000
      )
    );

comment on column public.store_settings.standard_delivery_paisa is
  'Flat delivery fee in paisa (1/100 PKR). Used when free-delivery threshold is not met.';

comment on column public.store_settings.free_delivery_min_subtotal_paisa is
  'When set, delivery is Rs 0 if order merchandise subtotal (paisa) is >= this value.';

update public.store_settings
set
  standard_delivery_paisa = coalesce(standard_delivery_paisa, 50000)
where id = 1;
...
declare
  ...
  v_free_delivery_threshold_paisa int;
...
begin
  ...
  v_free_delivery_threshold_paisa := (
    select ss.free_delivery_min_subtotal_paisa from public.store_settings ss where ss.id = 1
  );

  if
    v_free_delivery_threshold_paisa is not null
    and v_subtotal_cents >= v_free_delivery_threshold_paisa::bigint
  then
    v_shipping_cents := 0;
  else
...
comment on function public.place_order(jsonb) is
  'Checkout: shipping from store_settings (standard_delivery_paisa, optional free threshold); sellable stock.';
