-- Per-product free delivery (opt-in). Shipping tiers apply only to merchandise that is not free_delivery.
...
comment on column public.products.free_delivery is
  'When true, this product line total is excluded from shipping and from free-delivery threshold basis.';
...
comment on function public.place_order(jsonb) is
  'Checkout: PKR delivery; per-product free_delivery excludes line from shipping basis; optional voucher_code redeems in same transaction; snapshots include voucher summary.';
