-- Harden checkout inventory: sellable = on_hand - reserved (matches storefront).
-- Backfill missing inventory rows; keep products.stock_total in sync with variant sums.

-- ---------------------------------------------------------------------------
-- 1) Ensure every variant has an inventory row (avoids place_order UPDATE no-op)
-- ---------------------------------------------------------------------------
insert into public.inventory (product_variant_id, quantity_on_hand, quantity_reserved, updated_at)
select v.id, 0, 0, now()
from public.product_variants v
where not exists (
  select 1 from public.inventory i where i.product_variant_id = v.id
);

-- ---------------------------------------------------------------------------
-- 2) place_order: validate and decrement using sellable quantity
-- ---------------------------------------------------------------------------
...
comment on function public.place_order(jsonb) is
  'Checkout: validates sellable stock (on_hand - reserved), decrements on_hand; rolls back on failure.';

-- ---------------------------------------------------------------------------
-- 3) Keep products.stock_total = sum(inventory.quantity_on_hand) per product
-- ---------------------------------------------------------------------------
create or replace function public.tg_sync_product_stock_total()
returns trigger
language plpgsql
security definer
set search_path = public
...
declare
  ...
  vid uuid;
begin
  vid := coalesce(new.product_variant_id, old.product_variant_id);
  if vid is null then
    return coalesce(new, old);
  end if;

  select v.product_id into pid
  from public.product_variants v
  where v.id = vid
  limit 1;
  ...
  set
    stock_total = coalesce(
      (
        select sum(i.quantity_on_hand)::int
        from public.product_variants v2
        inner join public.inventory i on i.product_variant_id = v2.id
...
drop trigger if exists trg_inventory_sync_stock_total on public.inventory;

create trigger trg_inventory_sync_stock_total
  after insert or update or delete
  on public.inventory
  for each row
  execute function public.tg_sync_product_stock_total();

comment on function public.tg_sync_product_stock_total() is
  'Recomputes products.stock_total from sum of variant inventory.quantity_on_hand.';

-- One-time realign stock_total (trigger covers future changes)
update public.products p
set stock_total = coalesce(
  (
    select sum(i.quantity_on_hand)::int
    from public.product_variants v
    inner join public.inventory i on i.product_variant_id = v.id
