-- Standard delivery currency (ISO) + multiple free-delivery minimums as JSON array of minor units (PKR).

alter table public.store_settings add column if not exists standard_delivery_currency text;

update public.store_settings
set standard_delivery_currency = 'PKR'
where id = 1 and (standard_delivery_currency is null or btrim(standard_delivery_currency) = '');

alter table public.store_settings alter column standard_delivery_currency set default 'PKR';

alter table public.store_settings alter column standard_delivery_currency set not null;

alter table public.store_settings drop constraint if exists store_settings_std_delivery_currency_chk;

alter table public.store_settings
  add constraint store_settings_std_delivery_currency_chk
  check (standard_delivery_currency in ('PKR', 'USD', 'EUR'));

alter table public.store_settings add column if not exists free_delivery_thresholds_paisa jsonb not null default '[]'::jsonb;
...
begin
  if exists (
    ...
    where table_schema = 'public'
      ...
      and column_name = 'free_delivery_min_subtotal_paisa'
  ) then
    update public.store_settings
    set free_delivery_thresholds_paisa = case
      when free_delivery_min_subtotal_paisa is not null
      then jsonb_build_array(free_delivery_min_subtotal_paisa::bigint)
      else '[]'::jsonb
    end
    where id = 1;

    alter table public.store_settings drop column free_delivery_min_subtotal_paisa;
  end if;
end $$;
...
comment on function public.place_order(jsonb) is
  'Checkout: PKR delivery from store_settings; optional free rules (JSON array of minor units).';
