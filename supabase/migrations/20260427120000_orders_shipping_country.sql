-- Store ISO country code on orders (checkout is Pakistan-only; UI sends PK).

alter table public.orders
  add column if not exists shipping_country text not null default 'PK';

comment on column public.orders.shipping_country is 'ISO 3166-1 alpha-2 country code for shipping (e.g. PK).';
...
comment on function public.place_order(jsonb) is
  'Creates an order from checkout JSON: items[{variant_id, quantity}], customer + shipping fields (shipping_country ISO code, default PK). Guest or signed-in.';
